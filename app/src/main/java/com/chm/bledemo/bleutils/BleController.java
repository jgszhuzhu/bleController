...
import com.chm.bledemo.bleutils.callback.BleDevceScanCallback;
...
public class BleController {
    ...
    private final int GATTMAXLEN = 20;
    //此属性一般不用修改
    ...
    public void WriteBuffer(String value, OnWriteCallback writeCallback) {
        ...
        //对于过长的拆包分次发送
        byte[] remainString = HexUtil.hexStringToBytes(value);
        try{
            while (remainString.length > 0) {
                Thread.sleep(20);
                remainString = writeByGatt(remainString);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
...
//        boolean b = mBleGatt.writeCharacteristic(mBleGattCharacteristic);
    ...
    private byte[] writeByGatt(byte[] messageByte) {
        int length = messageByte.length;
        byte[] remainString, sendString;

        if (length > GATTMAXLEN) {
            sendString = Arrays.copyOfRange(messageByte, 0, GATTMAXLEN);
            remainString = Arrays.copyOfRange(messageByte, GATTMAXLEN, length);
        } else {
            sendString = messageByte;
            remainString = new byte[0];
        }
        mBleGattCharacteristic.setValue(sendString);
